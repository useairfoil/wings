...
use crate::{
    cluster_metadata::{ClusterMetadata, ClusterMetadataError},
...
async fn create_aws_s3_store(
    config: &crate::resources::AwsConfiguration,
...
async fn create_azure_blob_store(
    config: &crate::resources::AzureConfiguration,
) -> Result<Arc<dyn ObjectStore>> {
...
async fn create_google_cloud_store(
    config: &crate::resources::GoogleConfiguration,
) -> Result<Arc<dyn ObjectStore>> {
...
async fn create_s3_compatible_store(
    config: &crate::resources::S3CompatibleConfiguration,
) -> Result<Arc<dyn ObjectStore>> {
    use object_store::aws::AmazonS3Builder;
...
impl Error {
    pub fn kind(&self) -> crate::ErrorKind {
        match self {
            ...
            Self::Creation { .. } => crate::ErrorKind::Internal,
            Self::UnsupportedConfiguration => crate::ErrorKind::Validation,
        }
    }
}
