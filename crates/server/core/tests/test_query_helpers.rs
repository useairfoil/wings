...
use wings_server_core::query::helpers::{find_partition_column_value, validate_seqnum_filters};

#[test]
fn test_validate_seqnum_filters_with_eq() {
    // Test single equality filter
    let filters = vec![Expr::eq(
        col(SEQNUM_COLUMN_NAME),
        Expr::Literal(ScalarValue::UInt64(Some(5)), None),
    )];
    let result = validate_seqnum_filters(&filters).expect("should parse eq filter");
    assert_eq!(result, RangeInclusive::new(5, 5));
}

#[test]
fn test_validate_seqnum_filters_with_between() {
    ...
    let result = validate_seqnum_filters(&filters).expect("should parse between filter");
    assert_eq!(result, RangeInclusive::new(10, 100));
}

#[test]
fn test_validate_seqnum_filters_with_gt_and_lt() {
    ...
    let result = validate_seqnum_filters(&filters).expect("should parse gt/lt filter");
    assert_eq!(result, RangeInclusive::new(10, 100));
}

#[test]
fn test_validate_seqnum_filters_reversed_bounds() {
    ...
    let result = validate_seqnum_filters(&filters).expect("should parse reversed bounds filter");
    assert_eq!(result, RangeInclusive::new(10, 100));
}

#[test]
fn test_validate_seqnum_filters_no_filter() {
    let filters: Vec<Expr> = vec![];
    let err = validate_seqnum_filters(&filters).unwrap_err();
...
#[test]
fn test_validate_seqnum_filters_invalid_multiple_bounds() {
    ...
    insta::assert_debug_snapshot!(err, @r#"
    Plan(
        "Invalid __seqnum__ filter provided. You must provide a lower and upper bound for __seqnum__.",
    )
    "#);
}

#[test]
fn test_validate_seqnum_filters_duplicate_lower() {
